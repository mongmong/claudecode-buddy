#!/usr/bin/env node
// Stop-hook review gate for the codex plugin.
//
...
const { dispatchCodex } = await import("./lib/review-dispatch.mjs");
const { detectCodex } = await import("./lib/cli-detection.mjs");
...
async function main() {
  ...
  if (!cfg.value.gate?.enabled) process.exit(0); // opt-out path: gate is OFF
  ...
  if (!cli.installed) {
    logWarn(`codex not installed; failing open`);
    process.exit(0);
  }
  ...
  try {
    result = await Promise.race([
      dispatchCodex({
        ...
        model: null,
        variant: null,
        sandbox: "read-only", // review-only; never writes
        prompt,
        noSession: true, // Stop-hook gate doesn't accumulate session history
      }),
